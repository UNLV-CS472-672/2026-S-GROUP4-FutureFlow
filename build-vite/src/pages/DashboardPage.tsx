...
export default function DashboardPage() {
  ...
  // Load from localStorage; fall back to FEATURED_JOBS so carousels always have data
  useEffect(() => {
  ...
  const JobCard = ({ job }: { job: Job }) => (
    <div
      ...
      className="group bg-white border border-gray-200 rounded-2xl p-5 hover:shadow-md hover:border-green-300 hover:-translate-y-1 transition-all duration-200 cursor-pointer"
    >
      ...
      <h4 className="text-base font-semibold mb-3 group-hover:text-green-700 leading-tight line-clamp-2">
        {job.title}
      </h4>
      <div className="space-y-1.5 text-xs text-gray-500">
        <div className="flex items-center gap-1.5"><MapPin size={12} className="shrink-0" /><span className="truncate">{job.location}</span></div>
        <div className="flex items-center gap-1.5"><Briefcase size={12} className="shrink-0" /><span>{job.type}</span></div>
        <div className="flex items-center gap-1.5"><DollarSign size={12} className="shrink-0" /><span>{job.salary}</span></div>
      </div>
      ...
      <div className="flex gap-1.5">
        {Array.from({ length: total }).map((_, i) => (
          <button
            key={i}
            onClick={() => onDot(i)}
            className={`w-2 h-2 rounded-full transition-colors ${i === page ? 'bg-green-600' : 'bg-gray-300 hover:bg-gray-400'}`}
          />
        ))}
      </div>
      <button onClick={onPrev} disabled={page === 0} className="p-1 border rounded-full disabled:opacity-30 hover:border-green-400 transition-colors">
        <ChevronLeft size={16} />
      </button>
      <button onClick={onNext} disabled={page === total - 1} className="p-1 border rounded-full disabled:opacity-30 hover:border-green-400 transition-colors">
  ...
  return (
    <div className="min-h-screen bg-[#eeede9] pt-5">
      <AuthHeader title="Dashboard" />

      <div className="px-8 py-6 grid grid-cols-[minmax(380px,1fr)_3fr] gap-5 items-start">

      {/* LEFT COLUMN */}
      <div className="flex flex-col gap-5 h-full">
      <div className="bg-white shadow-md rounded-3xl border border-gray-200 p-6 flex flex-col flex-grow justify-between items-center text-center">
        ...
        {/* PROGRESS RING */}
        <div className="relative w-56 h-56 my-3">
            <svg className="w-56 h-56 -rotate-90">
              <circle cx="112" cy="112" r="85" stroke="#f0f0f0" strokeWidth="16" fill="none" />
              <circle cx="112" cy="112" r="85" stroke="#1D9E75" strokeWidth="16" fill="none"
                strokeLinecap="round" strokeDasharray={2 * Math.PI * 85} strokeDashoffset={2 * Math.PI * 85 * (1 - progress / 100)}
              />
            </svg>
          ...
          <div className="flex gap-2 w-full mt-3">
            {[
              ...
              <div key={stat.label} className="flex-1 bg-gray-100 rounded-md p-3">
        ...
        {/* Documents */}
        <div className="bg-white shadow-md rounded-3xl border border-gray-200 p-6 flex flex-col gap-3">
        ...
        {/* RIGHT COLUMN */}
        <div className="flex flex-col gap-5 bg-white rounded-3xl p-6 shadow-md border border-gray-100">
          <h2 className="text-2xl font-bold text-gray-800 mb-5">Job Overview</h2>
          ...
          <div className="bg-gray-50 border border-gray-200 rounded-2xl p-5 mb-4">
            <div className="flex items-center justify-between mb-4">
              ...
              <CarouselNav
                page={savedPage} total={savedTotal}
            ...
            <div className="grid grid-cols-3 gap-4">
              {savedVisible.length > 0 ? (
                ...
                <div className="col-span-3 text-center text-gray-400 py-8 border-2 border-dashed border-gray-200 rounded-2xl text-sm">
                  No saved jobs yet
                </div>
              )}
            </div>
          ...
          <div className="bg-gray-50 border border-gray-200 rounded-2xl p-5">
            <div className="flex items-center justify-between mb-4">
              ...
              <CarouselNav
                page={appliedPage} total={appliedTotal}
